<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [scripthost](./scripthost.md) &gt; [ScriptFunctionScope](./scripthost.scriptfunctionscope.md)

## ScriptFunctionScope interface

The scope in which a script function is called

<b>Signature:</b>

```typescript
export interface ScriptFunctionScope 
```

## Properties

|  Property | Modifiers | Type | Description |
|  --- | --- | --- | --- |
|  [context](./scripthost.scriptfunctionscope.context.md) | <code>readonly</code> | unknown | The context, if any, that was given as a script evaluation option. |
|  [idempotent](./scripthost.scriptfunctionscope.idempotent.md) | <code>readonly</code> | boolean | Specifies whether the function is called in an idempotent scope, in which case it is not allowed to have any side-effects. |
|  [key](./scripthost.scriptfunctionscope.key.md) | <code>readonly</code> | string | Key of the function that is being invoked |
|  [onObserverExit?](./scripthost.scriptfunctionscope.onobserverexit.md) |  | (callback: () =&gt; void) =&gt; boolean | <i>(Optional)</i> Registers a callback function that shall be invoked when the observer, if any, of the current script evaluation is completed and therefore no longer observing the evaluation. |
|  [onScriptExit?](./scripthost.scriptfunctionscope.onscriptexit.md) |  | (callback: () =&gt; void) =&gt; boolean | <i>(Optional)</i> Registers a callback function that shall be invoked when the current script evaluation completes |

## Methods

|  Method | Description |
|  --- | --- |
|  [invalidate()](./scripthost.scriptfunctionscope.invalidate.md) | Invalidates the script evaluation that invoked the function |

